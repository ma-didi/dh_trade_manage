...
import com.giao.ssm.utill.file.PoiUtil;
...
public class ContractPrint {
    public void print(ContractCVO contract, String path, HttpServletResponse response) throws Exception {
        //相同厂家的信息一起打印
        List<ContractProductVO> oList = contract.getContractProductVOS();
        ...
        for (int i = 0; i < oList.size(); i++) {
            ...
            pageMap.put("Factory", "生产工厂：" + oProduct.getFactoryC().getFactoryName());
            pageMap.put("ContractNo", "合 同 号：" + contract.getContractNo());
            pageMap.put("Contacts", "联 系 人：" + oProduct.getFactoryC().getContactor());
            pageMap.put("SigningDate", "签单日期：" + UtilFuns.formatDateTimeCN(UtilFuns.dateTimeFormat(contract.getSigningDate())));
            pageMap.put("Phone", "电    话：" + UtilFuns.convertNull(oProduct.getFactoryC().getPhone()));
            ...
            pageMap.put("Crequest", "  " + UtilFuns.convertNull(contract.getRequest()));
            ...
            oldFactory = oProduct.getFactoryC().getFactoryName();
            ...
            if (contract.getPrintStyle().equals("2")) {
                ...
                if (i < oList.size()) {
                    ...
                    if (oProduct.getFactoryC().getFactoryName().equals(oldFactory)) {    //厂家不同另起新页打印，除去第一次的比较
...
//		sheet.removeRowBreak(6);
        ...
        for (int p = 0; p < pageList.size(); p++) {
            ...
            //设置logo图片
            poiUtil.setPicture(wb, patriarch, path + "make/xlsprint/305133.jpg", curRow, 2, curRow + 4, 2);
            ...
            if (UtilFuns.isNotEmpty(printMap.get("ProductImage"))) {
                System.out.println("图片"+printMap.get("ProductImage"));
                poiUtil.setPicture(wb, patriarch, path + "ufiles/jquery/ProductImage.jpg", curRow - 1, 1, curRow, 3);
            }
            ...
            if (contract.getPrintStyle().equals("2") && UtilFuns.isNotEmpty(printMap.get("ProductNo2"))) {
                ...
                //插入产品图片
                if (UtilFuns.isNotEmpty(printMap.get("ProductImage.jpg"))) {
                    System.out.println(printMap.get("ProductImage.jpg"));
                    poiUtil.setPicture(wb, patriarch, path + "ufiles/jquery/ProductImage.jpg", curRow - 1, 1, curRow, 3);
                }
